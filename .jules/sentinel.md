...
## 2024-06-25 - Prevent Command Injection in `run_local_runtime_bridge.py`
...
## 2024-06-25 - Prevent Path Traversal in local runtime bridge
**Vulnerability:** In `run_local_runtime_bridge.py`, the `run_faceswap_job` function accepted user-provided `saveDirectory` and `outputName` and used them directly with `Path()` object concatenation. This allowed an attacker to input relative paths like `../../../etc/passwd` to overwrite arbitrary files on the local machine when running FaceFusion jobs.
**Learning:** Concatenating user input with local paths is dangerous, especially when the resulting object is not verified against an allowed base directory. `pathlib.Path` alone does not inherently prevent path traversal if the user supplies absolute paths or enough `../` to escape the root directory context.
**Prevention:** Always validate resolved paths against an expected base directory using `os.path.commonpath`. By converting the intended root and the constructed path to absolute paths and checking their common path, we securely guarantee that the target file does not escape its designated folder.
