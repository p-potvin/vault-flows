...
## 2026-05-15 - Prevent Command Injection in `run_local_runtime_bridge.py`
...
## 2026-05-15 - Prevent XSS in HTML string templates
...
## 2025-05-14 - Prevent Path Traversal by Explicitly Verifying Boundaries
**Vulnerability:** The `run_local_runtime_bridge.py` models scanning handler allowed users to supply an arbitrary `modelsDir` directory, exposing potential path traversal and unauthorized directory scanning. In addition, the file saving mechanism used `os.path.commonpath`, which is insufficient to fully mitigate path traversal.
**Learning:** `os.path.commonpath` or simply resolving a path doesn't inherently prevent escaping out of boundaries. Standard path traversal mitigation requires resolving the user-provided path and the strict base boundary, and explicitly checking the relationship using `.is_relative_to()`.
**Prevention:** Always restrict paths to a defined base boundary (e.g., via `--allowed-models-dir`). When accepting a filename or path from a user payload that will be appended to a base directory, resolve it and validate it strictly using `pathlib.Path(user_path).resolve().is_relative_to(pathlib.Path(base_boundary).resolve())` to guarantee it does not escape.

## 2026-05-15 - Restrict CORS Configuration in Local Bridge
**Vulnerability:** The `run_local_runtime_bridge.py` backend used an unconditionally permissive CORS configuration (`Access-Control-Allow-Origin: *`) across all HTTP methods. This means that a malicious website visited by the user could trigger cross-origin requests to `http://localhost:8484` and silently execute commands or interact with the filesystem (e.g. via ComfyUI model scanning or `facefusion` execution).
**Learning:** Returning `Access-Control-Allow-Origin: *` blindly on local APIs exposes local machine resources and tools directly to external web contexts via Cross-Site Request Forgery / CORS exploitation. This is specifically dangerous for local bridges that control filesystem components or run local binaries.
**Prevention:** Use an `--allowed-origins` command-line argument (defaulting to known local development origins like `http://localhost:5173`) and parse it at application startup to populate a whitelist. Always validate the incoming `Origin` header against this whitelist before echoing it in the `Access-Control-Allow-Origin` header.
